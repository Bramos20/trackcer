...
export default function SharedArtistTracks({ producer, artist, 
  sharedTracks,
   auth }) {
  return (
    <AppLayout user={auth.user}>
      <Head title={`Shared Tracks - ${producer?.name} & ${artist}`} />
      ...
      <div className="space-y-6">
        ...
        <div className="relative bg-[#F4F4F4]/[0.86] dark:bg-[#191919]/[0.51] rounded-[27.35px] sm:rounded-[40px] p-6 sm:p-8 pb-0 overflow-hidden">
          <h1 className="text-2xl sm:text-3xl font-normal mb-6">
            Tracks shared by {producer?.name} & {artist}
          </h1>

          {sharedTracks?.length === 0 ? (
            <div className="flex items-center justify-center py-20">
              <p className="text-muted-foreground text-center">
                No shared tracks found between {producer?.name} and {artist}.
              </p>
            </div>
          ) : (
